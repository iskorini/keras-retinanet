...
def translate_x(img, pixels, replace):
  ...
  if isinstance(img, np.ndarray):
      img = Image.fromarray(img)
  img = img.transform(img.size, Image.AFFINE, (1, 0, pixels, 0, 1, 0), fillcolor=tuple(replace))
  return np.array(img)
...
def translate_y(img, pixels, replace):
  ...
  img = img.transform(img.size, Image.AFFINE, (1, 0, 0, 0, 1, pixels), fillcolor=tuple(replace))
  return np.array(img)
...
def _shift_bbox(bbox, image_height, image_width, pixels, shift_horizontal):
  ...
  if max_x < 0. or min_x > 1.0 or max_y < 0. or min_y > 1.0:
    #print("madonna puttana")
    return None
  if max_x <= min_x or max_y <= min_y: #fix'd?
    return None
  # Clip the bboxes to be sure the fall between [0, 1].
  min_y, min_x, max_y, max_x = _clip_bbox(min_y, min_x, max_y, max_x)
  min_y, min_x, max_y, max_x = _check_bbox_area(min_y, min_x, max_y, max_x)
...
def translate_bbox(image, bboxes, pixels, replace, shift_horizontal):
  ...
  """
  #print("translate_bbox: shift_horizontal "+str(shift_horizontal))
  #print(bboxes)
  if shift_horizontal:
  ...
  #print("after shift of "+str(pixels)+"px")
  #print(bboxes)
  return image, bboxes
...
def shear_x(img, magnitude, fillcolor):
  ...
  if isinstance(img, np.ndarray):
      img = Image.fromarray(img)
  img = img.transform(img.size, Image.AFFINE, (1, magnitude, 0, 0, 1, 0), Image.BICUBIC, fillcolor=tuple(fillcolor))
  return np.array(img)
...
def shear_y(img, magnitude, fillcolor):
  ...
  if isinstance(img, np.ndarray):
      img = Image.fromarray(img)
  img = img.transform(img.size, Image.AFFINE, (1, 0, 0, magnitude, 1, 0), Image.BICUBIC, fillcolor=tuple(fillcolor))
  return np.array(img)
